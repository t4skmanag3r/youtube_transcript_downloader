...
def _get_serialized_share_entity(url: str) -> Optional[str]:
    try:
        data = req.get(url)
    ...
    if not search:
        return None
    param = str(data.content)[search.start() : search.end()].split(":")[1].strip('"')
    return param


def _get_innertube_api_key(url: str) -> Optional[str]:
    try:
        data = req.get(url)
    except Exception as e:
        raise e
...
def _get_transcript_json(param: str, key: str) -> Optional[Dict[Any, Any]]:
    PAYLOAD = {
        "context": {
            "client": {
...
def __navigate_to_cuegroups(json: dict) -> Dict[Any, Any]:
    return json["actions"][0]["updateEngagementPanelAction"]["content"][
        "transcriptRenderer"
    ]["body"]["transcriptBodyRenderer"]["cueGroups"]


def _parse_cuegroup(cue_group: dict) -> Tuple[str, str]:
    time = cue_group["transcriptCueGroupRenderer"]["formattedStartOffset"]["simpleText"]
    text = cue_group["transcriptCueGroupRenderer"]["cues"][0]["transcriptCueRenderer"][
        "cue"
    ]["simpleText"]
    return time, text


def _parse_transcript_json(json: dict):
    json = __navigate_to_cuegroups(json)

    transcript = {}
    for cue_group in json:
        time, text = _parse_cuegroup(cue_group)
        transcript[time] = text
    return transcript


def get_transcript(url: str) -> Optional[Dict[str, str]]:
    param = _get_serialized_share_entity(url)
    key = _get_innertube_api_key(url)
    if not param or not key:
        return None

    transcript_json = _get_transcript_json(param, key)
    if not transcript_json:
        return None

    return _parse_transcript_json(transcript_json)
